...
{
    srp_N = new BigInteger(srp_Nstr, 16); //str2bigInt(srp_Nstr, 16, 0);
    srp_g = new BigInteger("2");//str2bigInt("2", 10, 0);
    srp_k = new BigInteger("c46d46600d87fef149bd79b81119842f3c20241fda67d06ef412d8f6d9479c58", 16);//str2bigInt("c46d46600d87fef149bd79b81119842f3c20241fda67d06ef412d8f6d9479c58", 16, 0);
...
function srp_identify()
{
    ...
    // A = g**a % N
    srp_A = srp_g.modPow(srp_a, srp_N); //powMod(srp_g,srp_a,srp_N);
    srp_I = document.getElementById("srp_username").value;
...
{    
    //S -> C: s | B
    srp_B = new BigInteger(B, 16); //str2bigInt(B, 16, 0);
    srp_Bstr = B;
    // u = H(A,B)
    srp_u = new BigInteger(SHA256(srp_Astr + srp_Bstr), 16); //str2bigInt(SHA256(srp_Astr + srp_Bstr), 16, 0);    
    // x = H(s, H(I:p))
    srp_x = srp_calculate_x(s);
    //S = (B - kg^x) ^ (a + ux)
    var kgx = srp_k.multiply(srp_g.modPow(srp_x, srp_N));  //mult(srp_k, powMod(srp_g, srp_x, srp_N));
    var aux = srp_a.add(srp_u.multiply(srp_x)); //add(srp_a, mult(srp_u, srp_x));
    srp_S = srp_B.subtract(kgx).modPow(aux, srp_N); // powMod(sub(srp_B, kgx), aux, srp_N);
    // M = H(H(N) xor H(g), H(I), s, A, B, K)
    var Mstr = srp_A.toString(16) + srp_B.toString(16) + srp_S.toString(16); //bigInt2str(srp_A, 16) + bigInt2str(srp_B,16) + bigInt2str(srp_S,16);
    srp_M = SHA256(Mstr);
    srp_send_hash(srp_M);
    //M2 = H(A, M, K)
    srp_M2 = SHA256(srp_A.toString(16) + srp_M + srp_S.toString(16)); // SHA256(bigInt2str(srp_A, 16)+srp_M+bigInt2str(srp_S, 16));
};
